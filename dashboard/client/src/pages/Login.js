...
const Login = () => {
    ...
    if (!loading && user)
        return <react_router_dom_1.Navigate to="/portal"/>;
    const handleLogin = () => {
        window.location.href = `${import.meta.env.VITE_API_URL || "https://r3nder-api.onrender.com"}/auth/discord`;
    };
    return (<div className="h-screen w-screen flex items-center justify-center bg-background px-4">
            <div className="glass-card max-w-md w-full p-10 text-center space-y-8">
                ...
                <button onClick={handleLogin} className="premium-btn w-full flex items-center justify-center gap-3">
