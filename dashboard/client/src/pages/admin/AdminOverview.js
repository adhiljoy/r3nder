...
const AdminOverview = () => {
    ...
    (0, react_1.useEffect)(() => {
        ...
        axios_1.default.get(`${API_URL}/api/admin/analytics/overview`, { withCredentials: true })
            .then(res => {
            setStats(res.data);
            setLoading(false);
        })
    ...
    if (loading)
        return <div className="animate-pulse text-white/50">Loading Admin Stats...</div>;
    if (!stats)
        return <div className="text-red-400">Failed to load admin dashboard. Restricted Access.</div>;
    const kpis = [
        { label: "Total Logs", value: stats.totalLogs, icon: <lucide_react_1.Terminal size={20}/>, color: "text-blue-400", bg: "bg-blue-400/10" },
        { label: "Total Servers", value: stats.totalGuilds, icon: <lucide_react_1.Layers size={20}/>, color: "text-purple-400", bg: "bg-purple-400/10" },
        { label: "Total Users", value: stats.totalUsers, icon: <lucide_react_1.Users size={20}/>, color: "text-green-400", bg: "bg-green-400/10" },
        { label: "System Health", value: "99.9%", icon: <lucide_react_1.Activity size={20}/>, color: "text-emerald-400", bg: "bg-emerald-400/10" },
    ];
    const chartData = stats.statsByType.map((s) => ({
    ...
    return (<div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-4xl font-black tracking-tighter text-white flex items-center gap-3">
                        <lucide_react_1.ShieldAlert className="text-red-500" size={32}/>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {kpis.map((kpi, i) => (<div key={i} className="glass-card p-6 rounded-3xl border border-white/5 group hover:border-white/10 transition-all">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 glass-card p-8 rounded-4xl border border-white/5">
                    <div className="flex items-center justify-between mb-8">
                        <h2 className="text-xl font-bold text-white flex items-center gap-2">
                            <lucide_react_1.Zap size={20} className="text-primary"/>
                    ...
                    <div className="h-[300px]">
                        <recharts_1.ResponsiveContainer width="100%" height="100%">
                            <recharts_1.BarChart data={chartData}>
                                <recharts_1.XAxis dataKey="name" stroke="#ffffff20" fontSize={12} tickLine={false} axisLine={false}/>
                                <recharts_1.Tooltip contentStyle={{ backgroundColor: "#000", border: "1px solid #ffffff10", borderRadius: "12px", color: "#fff" }} cursor={{ fill: "#ffffff05" }}/>
                                <recharts_1.Bar dataKey="count" radius={[8, 8, 0, 0]}>
                                    {chartData.map((_entry, index) => (<recharts_1.Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]}/>))}
                                </recharts_1.Bar>
                            </recharts_1.BarChart>
                ...
                <div className="glass-card p-8 rounded-4xl border border-white/5 bg-red-500/5">
                    <h2 className="text-xl font-bold text-white mb-6 flex items-center gap-2">
                        <lucide_react_1.AlertCircle size={20} className="text-red-500"/>
                        Active Incidents
                    </h2>
