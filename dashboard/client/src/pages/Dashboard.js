...
const Dashboard = () => {
    const { guildId, tab } = (0, react_router_dom_1.useParams)();
    const [activeTab, setActiveTab] = (0, react_1.useState)(tab || "ai");
    ...
    const tabs = [
        { id: "ai", label: "AI Control", icon: <lucide_react_1.Zap size={18}/> },
        { id: "moderation", label: "Moderation", icon: <lucide_react_1.Shield size={18}/> },
        { id: "music", label: "Music Flux", icon: <lucide_react_1.Music size={18}/> },
        { id: "analytics", label: "Nexus Stats", icon: <lucide_react_1.Activity size={18}/> },
        { id: "settings", label: "Executive", icon: <lucide_react_1.Settings size={18}/> }
    ];
    return (<div className="space-y-12 pb-32">
            {/* Header Section */}
            <header className="flex flex-col md:flex-row md:items-end justify-between gap-10">
                ...
                <div className="flex items-center gap-4">
                    <button onClick={saveSettings} disabled={saving} className="premium-btn py-3 px-10 flex items-center gap-3">
            ...
            <div className="flex flex-wrap gap-4 p-2 rounded-2xl glass-card border-white/5 w-fit">
                {tabs.map((tab) => (<react_router_dom_1.Link key={tab.id} to={`/app/${guildId}/${tab.id === "ai" ? "" : tab.id}`} className={`flex items-center gap-3 px-6 py-3 rounded-xl transition-all duration-500 font-bold uppercase tracking-widest text-[10px] ${activeTab === tab.id
            ...
            <framer_motion_1.AnimatePresence mode="wait">
                <framer_motion_1.motion.div key={activeTab} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.4 }} className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                    {activeTab === "ai" && (<>
                            <div className="glass-card p-10 space-y-8">
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="p-6 rounded-2xl bg-white/3 border border-white/5 flex items-center justify-between group hover:border-primary/20 transition-colors">
                                        ...
                                        <button onClick={() => setSettings({ ...settings, aiAutopilot: { ...settings.aiAutopilot, enabled: !settings.aiAutopilot?.enabled } })} className={`w-14 h-8 rounded-full transition-all duration-500 relative flex items-center px-1 ${settings.aiAutopilot?.enabled ? "bg-primary" : "bg-white/10"}`}>
                                    ...
                                    <div className="p-6 rounded-2xl bg-white/3 border border-white/5 space-y-4">
                                        <p className="text-[10px] font-black uppercase tracking-widest text-white/30 flex items-center gap-2"><lucide_react_1.Globe size={12}/> Personality Pulse Mode</p>
                                        <div className="grid grid-cols-3 gap-3">
                                            {["coder", "mod", "assistant"].map(p => (<button key={p} onClick={() => setSettings({ ...settings, aiAutopilot: { ...settings.aiAutopilot, personality: p } })} className={`py-3 rounded-xl border text-[10px] font-black uppercase transition-all duration-300 ${settings.aiAutopilot?.personality === p ? "bg-primary/10 border-primary text-primary" : "bg-white/5 border-white/5 text-white/20 hover:bg-white/10 hover:text-white"}`}>
                                                    {p}
                                                </button>))}
                                        </div>
                                    </div>
                    ...
                    {activeTab === "moderation" && (<>
                            <div className="glass-card p-10 space-y-8">
                                ...
                                <div className="space-y-6">
                                    <div className="p-5 rounded-xl bg-white/3 border border-white/5 flex items-center justify-between">
                                        <div className="flex items-center gap-4">
                                            <div className="p-3 bg-emerald-500/10 text-emerald-500 rounded-xl"><lucide_react_1.Lock size={18}/></div>
                                            <div>
                                    ...
                                    <div className="p-5 rounded-xl bg-white/3 border border-white/5 flex items-center justify-between">
                                        <div className="flex items-center gap-4">
                                            <div className="p-3 bg-amber-500/10 text-amber-500 rounded-xl"><lucide_react_1.Bell size={18}/></div>
                                            <div>
                            ...
                            <div className="glass-card p-10 bg-linear-to-br from- emerald-500/5 to-transparent space-y-6">
                                ...
                                <div className="space-y-3">
                                    {["Link Filter", "Spam Pulse Control", "Explicit Sync Guard"].map(item => (<div key={item} className="p-4 rounded-xl bg-white/3 border border-white/5 flex items-center justify-between">
                            ...
                            <div className="glass-card p-10 space-y-10 lg:col-span-2">
                                <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
                                    ...
                                    <div className="flex items-center gap-4 bg-white/3 p-4 rounded-2xl border border-white/5">
                                        <lucide_react_1.Volume2 className="text-primary"/>
                                        <input type="range" className="w-44 accent-primary" onChange={(e) => setSettings({ ...settings, music: { ...settings.music, volume: parseInt(e.target.value) } })}/>
                                        <span className="text-xs font-black text-primary w-8">{settings.music?.volume}%</span>
                                    </div>
                                </div>
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                                    {["Auto-Play Sync", "Voice Pulse Lock", "SoundCloud Direct"].map(feature => (<div key={feature} className="p-6 rounded-2xl glass-card border-white/5 flex flex-col items-center justify-center text-center space-y-4 hover:border-primary/20 transition-all cursor-pointer group">
                                            <div className="p-4 rounded-full bg-white/3 text-white/20 group-hover:bg-primary/10 group-hover:text-primary transition-colors"><lucide_react_1.Music size={24}/></div>
                    ...
                    {activeTab === "analytics" && (<div className="glass-card p-10 lg:col-span-2 space-y-12 overflow-hidden relative">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                                {[
                { label: "Pulse Count", val: "12,482", icon: <lucide_react_1.Activity size={18}/>, color: "text-blue-500" },
                { label: "AI Response", val: "4.2s", icon: <lucide_react_1.Clock size={18}/>, color: "text-purple-500" },
                { label: "Unique Users", val: "842", icon: <lucide_react_1.Users size={18}/>, color: "text-emerald-500" },
                { label: "Command Flow", val: "3.1/min", icon: <lucide_react_1.BarChart3 size={18}/>, color: "text-amber-500" }
            ].map((item) => (<div key={item.label} className="p-6 rounded-2xl bg-white/3 border border-white/5 hover:border-white/10 transition-colors">
                            ...
                            <div className="h-64 rounded-2xl bg-white/3 border border-white/5 flex items-center justify-center text-white/20 font-black italic uppercase tracking-[0.5em] text-[10px] overflow-hidden">
                                ...
                                <div className="flex flex-col items-center gap-4 relative z-10">
                                    <lucide_react_1.Activity className="animate-pulse" size={48}/>
                                    <span>Orchestrating Historical Telemetry...</span>
                                </div>
                            </div>
                        </div>)}
                    {activeTab === "settings" && (<div className="lg:col-span-2 space-y-10">
                                    ...
                                    <div className="p-6 rounded-2xl bg-white/3 border border-white/5 space-y-6">
                                        ...
                                        <div className="space-y-4">
                                            <div className="flex flex-col gap-2">
                                                ...
                                                <input type="text" value={settings.nickname || ""} onChange={(e) => setSettings({ ...settings, nickname: e.target.value })} className="w-full bg-background border border-white/10 rounded-xl px-4 py-3 text-sm focus:border-primary outline-none transition-all" placeholder="Enter high-fidelity nickname..."/>
                                            </div>
                                        </div>
                                    </div>
                ...
                setSettings({ ...settings, reminders });
                                        ...
                                        <div className="space-y-4 max-h-[200px] overflow-y-auto pr-2 custom-scrollbar">
                                            {(settings.reminders || []).map((reminder, idx) => (<div key={reminder.id} className="flex items-center gap-3 p-3 rounded-xl bg-white/3 border border-white/5 group">
                                                    <div className="flex-1 space-y-1">
                                                        <input className="bg-transparent border-none p-0 text-xs font-bold w-full focus:outline-none" value={reminder.text} onChange={(e) => {
                    const reminders = [...settings.reminders];
                    ...
                    setSettings({ ...settings, reminders });
                                                    ...
                                                    <button onClick={() => {
                    const reminders = settings.reminders.filter((_, i) => i !== idx);
                    setSettings({ ...settings, reminders });
                }} className="text-white/10 hover:text-red-500 transition-colors">
                                                        <lucide_react_1.Trash2 size={14}/>
                                                    </button>
                                                </div>))}
                                            {(!settings.reminders || settings.reminders.length === 0) && (<p className="text-[10px] text-white/10 italic text-center py-4">No active event pulses detected.</p>)}
                                        </div>
                                    </div>
                                </div>
                            </div>
