...
const AIControl = () => {
    ...
    (0, react_1.useEffect)(() => {
        ...
        axios_1.default.get(`${API_URL}/api/guild/${guildId}/settings`, { withCredentials: true })
    ...
    if (!settings)
        return <div className="p-20 text-center text-white/40">Loading AI config...</div>;
    const personalities = [
    ...
    return (<div className="space-y-10 animate-in fade-in duration-700">
            <header className="flex items-center justify-between">
                ...
                <button onClick={handleSave} disabled={saving} className="premium-btn min-w-[160px]">
                ...
                <div className="lg:col-span-2 space-y-8">
                    <div className="glass-card p-8 space-y-10">
                        <div className="flex items-center gap-3 mb-4">
                            <lucide_react_1.Zap className="text-primary"/>
            ...
            { id: "inactiveEngage", label: "Inactivity Monitor", desc: "Re-engage quiet channels automatically.", field: "inactiveEngage" },
        ].map((toggle) => (<div key={toggle.id} className="flex items-start justify-between p-4 rounded-2xl bg-white/5 border border-white/5 hover:border-white/10 transition-colors">
                                    <div className="space-y-1">
                                        <p className="font-bold text-sm">{toggle.label}</p>
                                        <p className="text-xs text-white/40">{toggle.desc}</p>
                                    </div>
                                    <label className="relative inline-flex items-center cursor-pointer mt-1">
                                        <input type="checkbox" checked={settings.aiAutopilot[toggle.field]} onChange={(e) => setSettings({
                ...settings,
                aiAutopilot: { ...settings.aiAutopilot, [toggle.field]: e.target.checked }
                    ...
                    <div className="glass-card p-8 space-y-6">
                        <div className="flex items-center gap-3">
                            <lucide_react_1.Sparkles className="text-premium"/>
                            <h3 className="text-xl font-bold">Bot Personality</h3>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {personalities.map((p) => (<button key={p.id} onClick={() => setSettings({
                ...settings,
                aiAutopilot: { ...settings.aiAutopilot, personality: p.id }
            })} className={`p-6 rounded-2xl border text-left transition-all duration-300 ${settings.aiAutopilot.personality === p.id
                ...
                <div className="space-y-8">
                    <div className="glass-card p-8 bg-linear-to-br from-primary/10 to-transparent border-primary/20 flex flex-col gap-6">
                        <div className="flex items-center gap-3">
                            <lucide_react_1.ShieldAlert className="text-primary"/>
                    ...
                    <div className="glass-card p-8 space-y-4 border-yellow-500/20">
                        <div className="flex items-center gap-3">
                            <lucide_react_1.AlertCircle className="text-yellow-500" size={20}/>
