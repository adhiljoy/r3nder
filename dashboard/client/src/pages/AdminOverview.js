...
const AdminOverview = () => {
    ...
    (0, react_1.useEffect)(() => {
        axios_1.default.get("https://r3nder-api.onrender.com/api/admin/stats", { withCredentials: true })
            .then(res => setStats(res.data))
            .catch(() => { });
    }, []);
    const metrics = [
        { label: "High-Fidelity Nexus Count", val: stats.nexusCount, icon: <lucide_react_1.Server />, color: "text-blue-500", bg: "bg-blue-500/10" },
        { label: "Orchestrated Identities", val: "85.2k", icon: <lucide_react_1.Users />, color: "text-purple-500", bg: "bg-purple-500/10" },
        { label: "AI Neural Pulses", val: "1.2M", icon: <lucide_react_1.Cpu />, color: "text-amber-500", bg: "bg-amber-500/10" },
        { label: "Monolith System Health", val: stats.status, icon: <lucide_react_1.Activity />, color: "text-emerald-500", bg: "bg-emerald-500/10" }
    ];
    ...
    return (<div className="p-8 space-y-12 pb-44 h-screen overflow-auto custom-scrollbar">
            {/* Header */}
            <header className="flex flex-col md:flex-row md:items-center justify-between gap-8">
                ...
                <div className="flex items-center gap-4 p-2 rounded-2xl glass-card border-red-500/20">
                    <div className="flex items-center gap-2 px-4 py-2 rounded-xl bg-red-500/10 text-red-500 font-black uppercase text-[10px] tracking-widest animate-pulse">
                        <lucide_react_1.Activity size={14}/> LIVE MONITORING
                    </div>
                </div>
            </header>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                {metrics.map((m, i) => (<framer_motion_1.motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} key={m.label} className="glass-card p-8 border-white/5 space-y-6 glass-card-hover group">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="glass-card p-10 border-white/5 space-y-8 lg:col-span-2">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {pipelineStatus.map((p) => (<div key={p.name} className="flex items-center justify-between p-5 rounded-2xl bg-white/3 border border-white/5 group hover:bg-white/5 transition-colors border-l-4 border-l-transparent hover:border-l-red-500">
                                <div>
            ...
            <div className="glass-card p-10 border-white/5 space-y-10">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <lucide_react_1.TrendingUp className="text-red-500"/>
                ...
                <div className="h-48 flex items-end gap-3 px-4">
                    {[10, 45, 30, 70, 40, 90, 60, 85, 35, 55, 20, 80, 50, 65, 45, 95].map((h, i) => (<framer_motion_1.motion.div key={i} initial={{ height: 0 }} animate={{ height: `${h}%` }} transition={{ delay: i * 0.05, duration: 1 }} className="flex-1 bg-linear-to-t from-red-500/20 via-red-500/10 to-red-500 rounded-t-lg shadow-lg shadow-red-500/20 group relative cursor-pointer">
