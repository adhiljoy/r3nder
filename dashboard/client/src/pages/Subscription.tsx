...
const Subscription = () => {
    ...
    if (loading) return <div className="p-20 text-center text-white/40">Syncing subscription tokens...</div>;
    ...
    const tiers = [
        ...
        },
        {
            ...
            name: "Ultra",
            price: "$24.99",
            icon: <Crown className="text-premium" />,
        ...
        <div className="space-y-16 animate-in fade-in slide-in-from-bottom-10 duration-1000">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 max-w-6xl mx-auto px-4">
                {tiers.map((tier) => (
                    <div 
                        key={tier.id} 
                        className={`glass-card p-10 flex flex-col gap-8 relative overflow-hidden group transition-all duration-500 hover:scale-[1.03] ${tier.color}`}
                    >
                        {tier.id === "ultra" && (
                            <div className="absolute top-0 right-0 bg-premium px-6 py-1 rounded-bl-2xl text-[10px] font-black text-black uppercase tracking-widest">
                                Most Powerful
                            </div>
                        )}
                            ...
                            {tier.features.map((f, i) => (
                                <div key={i} className="flex items-center gap-3 text-sm">
                                    <div className="shrink-0 w-5 h-5 rounded-full bg-white/5 flex items-center justify-center">
                                        <Check size={12} className={tier.id === 'free' ? 'text-white/20' : 'text-primary'} />
                                    </div>
                        ...
                        <button 
                            disabled={currentTier === tier.id || (upgrading !== null)}
                            onClick={() => handleUpgrade(tier.id)}
                            className={`w-full py-4 rounded-xl font-black text-sm transition-all duration-300 ${
                                currentTier === tier.id 
