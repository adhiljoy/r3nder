...
const caption: Command = {
    data: new SlashCommandBuilder()
        ...
        .addAttachmentOption(option =>
            option.setName("image").setDescription("Upload the image to caption").setRequired(true))
        .addStringOption(option =>
            option.setName("style").setDescription("Caption style").setRequired(true)
                .addChoices(
                    { name: "Funny", value: "funny" }, { name: "Dark", value: "dark" },
                    { name: "Cool", value: "cool" }, { name: "Poetic", value: "poetic" },
                    { name: "Savage", value: "savage" }, { name: "Professional", value: "professional" },
    ...
    async execute(client: R3NDERClient, interaction: ChatInputCommandInteraction): Promise<void> {
        ...
        try {
            ...
            const captions = await client.vision.processImage(attachment.url,
                `Generate 3 creative captions in a ${style} style. Keep them short. Format as bullet points.`);
